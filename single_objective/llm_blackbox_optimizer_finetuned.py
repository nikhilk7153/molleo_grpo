...
class FinetunedLLMBlackBoxOptimizer:
    def __init__(self, args):
        self.args = args
        self.oracle = MolleoOracle(args=args)
        self.candidate_pool = []  # List of (smiles, score) tuples
        # Use fine-tuned model server (different port and model name)
        self.vllm_server_url = f"http://localhost:{args.finetuned_port}/v1/chat/completions"
        self.model_name = args.finetuned_model_name
        ...
        print(f"🧬 Using Fine-tuned Model Server:")
        print(f"   URL: {self.vllm_server_url}")
        print(f"   Model: {self.model_name}")
        
    def test_finetuned_server(self):
        """Test if the fine-tuned model server is running"""
        try:
            test_payload = {
                "model": self.model_name,
                "messages": [
                    {"role": "system", "content": "You are a molecular design assistant."},
                    {"role": "user", "content": "Generate a simple molecule: "}
                ],
                "temperature": 0.3,
                "max_tokens": 50
            }
            ...
            print(f"✅ Fine-tuned server test successful!")
            print(f"   Test response: {test_response[:100]}...")
            return True
            
        except Exception as e:
            print(f"❌ Fine-tuned server test failed: {e}")
            print(f"   Make sure the fine-tuned model server is running on port {self.args.finetuned_port}")
            return False
    ...
    def initialize_pool(self, n_samples: int):
        ...
        # Test fine-tuned server first
        if not self.test_finetuned_server():
            raise Exception("Fine-tuned server not available. Please start it first.")
    ...
    def query_llm(self, prompt: str, temperature: float = 0.7) -> str:
        """Query the fine-tuned LLM server"""
        messages = [
            {"role": "system", "content": "You are an expert molecular design assistant trained to generate high-quality molecules with improved properties. Generate molecules that are chemically valid, diverse, and optimized for the target property."},
        ...
        payload = {
            ...
            "max_tokens": 2048,
            "stop": ["User:", "System:", "Human:", "Assistant:"]
        }
        ...
        for retry in range(3):
            try:
                ...
                if retry == 2:
                    print(f"Failed to connect to fine-tuned model server on {self.vllm_server_url}")
                    raise e
            except Exception as e:
                print(f"Error querying fine-tuned LLM: {e}")
                if retry == 2:
                    raise e
    ...
    def optimize(self, n_init: int = 50000, m_per_iter: int = 50, max_iterations: int = 20, 
                 pool_sample_size: int = 200, use_weighted_sampling: bool = True):
        """Run the black-box optimization"""
        
        print("="*80)
        print("🚀 FINE-TUNED LLM BLACK-BOX OPTIMIZER")
        print("="*80)
        print(f"🧬 Using DPO-trained model: {self.model_name}")
        print(f"🌐 Server: {self.vllm_server_url}")
...
def main():
    parser = argparse.ArgumentParser(description='Fine-tuned LLM Black-box Optimizer for Molecular Design')
    ...
    parser.add_argument('--output_dir', type=str, default='./results_finetuned', help='Output directory')
    
    # Fine-tuned model specific arguments
    parser.add_argument('--finetuned_port', type=int, default=8001, 
                        help='Port where fine-tuned model server is running')
    parser.add_argument('--finetuned_model_name', type=str, default='dpo_finetuned',
                        help='Name of the fine-tuned model in vLLM server')
    ...
    # Create optimizer
    optimizer = FinetunedLLMBlackBoxOptimizer(args)
